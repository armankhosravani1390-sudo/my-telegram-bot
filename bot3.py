...
TOKEN = "8299446091:AAEUd52yP-7hKdtcjKFWIf5qqOhULnfm0Ls"
...
def build_admin_panel_markup():
    ...
    btn3 = telebot.types.InlineKeyboardButton("🏠 پنل اصلی بات", callback_data="back_to_main_user_panel")
...
def build_main_reply_keyboard(user_id):
    ...
    if is_founder(user_id) or is_owner(user_id):
        ...
        btn_manage = telebot.types.KeyboardButton("👑 پنل بات")

        markup.add(btn_manage)

    elif is_admin(user_id):

        btn_admin = telebot.types.KeyboardButton("⚙️ پنل ادمین")
...
def build_founder_markup():
    ...
    btn22 = telebot.types.InlineKeyboardButton("🏠 پنل اصلی بات", callback_data="back_to_main_user_panel")
...
def build_owner_markup():
    ...
    btn17 = telebot.types.InlineKeyboardButton("🏠 پنل اصلی بات", callback_data="back_to_main_user_panel")

    markup.add(btn1, btn2, btn3, btn4, btn5, btn6, btn7, btn8, btn9, btn11, btn12, btn13, btn14, btn15, btn16, btn17)
...
def handle_callbacks(call):
    ...
    if data == "back_to_main_user_panel":
...
@bot.message_handler(func=lambda m: m.text in ["🏠 پنل اصلی بات", "پنل اصلی بات", "پنل اصلی"])
...
@bot.message_handler(func=lambda m: m.text in ["👑 پنل بات", "پنل بات"])
...
@bot.message_handler(func=lambda m: m.text in ["⚙️ پنل ادمین", "پنل ادمین"])
...
def handle_messages(msg):
    ...
    if is_admin(user_id):
        ...
        if msg.text and not msg.text.startswith('/'):
            ...
            if is_founder(user_id) or is_owner(user_id):
                ...
                bot.reply_to(msg, "ℹ️ لطفاً بنویسید «پنل بات»")

            else:

                bot.reply_to(msg, "ℹ️ لطفاً بنویسید «پنل ادمین»")
    ...
    if user_id in waiting_for_message and waiting_for_message[user_id] == True:
        ...
        if msg.content_type == 'text' and msg.text and not msg.text.startswith('/'):
            ...
            bot.reply_to(msg, "ℹ️ لطفاً بنویسید «پنل اصلی»")
