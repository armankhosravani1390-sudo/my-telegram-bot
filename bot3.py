...
def get_ttt_board_markup(game_id, user_id):
    ...
    markup = telebot.types.InlineKeyboardMarkup(row_width=3)
    for i in range(9):
        callback_data = f"ttt_move_{game_id}_{i}"
        btn = telebot.types.InlineKeyboardButton(board[i], callback_data=callback_data)
        if i % 3 == 0:
            markup.row(btn)
        else:
            markup.row(btn)
...
def start_ttt_game(game_id):
    ...
    ttt_games[game_id]['turn'] = player1  # بازیکن اول شروع می‌کند
...
def handle_messages(msg):
    ...
    # ========== ورود با رمز Tic Tac Toe (دکمه‌ای) ==========
    ...
    # ========== ورود با رمز سنگ کاغذ قیچی (دکمه‌ای) ==========
...
def handle_callbacks(call):
    ...
    # ========== آپدیت بات (DIRECT - فقط یک بار) ==========
    elif call.data == "owner_update_direct":
        if user_id != OWNER_ID:
            bot.answer_callback_query(call.id, "⛔ فقط OWNER!")
            return
        
        # فقط یک بار ارسال میشه
        all_users = set()
        for user_id in waiting_for_message.keys():
            all_users.add(user_id)
        for user_id in tickets.keys():
