...
def games_menu_button(msg):
    ...
    btn2 = telebot.types.InlineKeyboardButton("🎲 بازی ۲", callback_data="game_coming_soon")
...
def handle_callbacks(call):
    ...
    if call.data.startswith("accept_ticket_"):
        ...
        # ========== ارسال پیام به کاربر با دستور /chat ==========
        bot.send_message(
            user_id_ticket, 
    ...
    # ========== آپدیت بات (DIRECT - اجرای /update) ==========
        ...
        # اجرای مستقیم دستور update (فقط یک بار)
        all_users = set()
        for user_id in waiting_for_message.keys():
            all_users.add(user_id)
        for user_id in tickets.keys():
